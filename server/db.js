 ...
 *
 * IMPORTANT: Firestore rejects writes containing an `undefined` field
 * value outright (throws, doesn't just skip it) — unlike `null`, which is
 * fine. If a caller ever forgets to pass limitPerDay/limitPerMonth, or a
 * future refactor drops one, this used to crash the whole request with an
 * unhelpful low-level Firestore error. Defaulting them to `null` here
 * means a missing value degrades to "unlimited" instead of a 500 — still
 * worth double-checking the caller passes the tier's real limits, but it
 * can no longer take the whole endpoint down.
 */
export async function createApiKey({ uid, label, key, preview, tier, limitPerDay = null, limitPerMonth = null }) {
...
/**
 * Lists a user's non-revoked API keys, most recent first.
 *
 * NOTE: this query filters on two fields (uid, revoked) AND orders by a
 * third (createdAt) — Firestore requires a composite index for that exact
 * combination and will NOT create one automatically. If this collection
 * doesn't have one yet, this call throws a "FAILED_PRECONDITION: The
 * query requires an index" error containing a direct link to create it.
 * That error is not a bug in this code — check your Render Application
 * logs for it and click the link, or create it manually in the Firebase
 * Console under Firestore → Indexes: apiKeys, fields uid (Asc), revoked
 * (Asc), createdAt (Desc).
 */
